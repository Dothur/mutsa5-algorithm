...
public class Q2503 {
    public int solution() throws IOException {
        ...
        for (int i = 0; i < tries; i++) {
            StringTokenizer countTokens = new StringTokenizer(br.readLine());
            ballInfo[i] = new int[]{
        ...
        for (int i = 1; i < 10; i++) { // 제일 앞자리 숫자 i
            for (int j = 1; j < 10; j++) { // 2번째 자리 숫자 j
                // j 와 i 는 같지 않아야한다.
                if (j == i) continue;
                for (int k = 1; k < 10; k++) { // 3번째 자리 숫자 k
                    if (k == i || k == j) continue;
                    // 답변 후보 배열
    ...
    public static void main(String[] args) throws IOException {
        System.out.println(new Q2503().solution());
    }
}
